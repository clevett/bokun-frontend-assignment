...
export async function createExperience(formData: FormData) {
  const id = uuidv4();
  const title = formData.get("title")?.toString() || "Walking tour";
  const description =
    formData.get("description")?.toString() || "Beautiful walking tour";
  ...
  const template = {
    id,
    title,
    rating: 9.5, //Assumption there would be a rating system for new tours. Leaving it as default for now.
    description,
    imageUrl,
  };
